...
\begin{figure}
	\includegraphics[width=1\columnwidth]{images/Flow_chart.png} 
...
\begin{figure}
	\centering
	\includegraphics[width=0.85\columnwidth]{images/All_types_of_DCCs.png} 
...
%(3) DATE 2018
\section{Boolean Encoding for DCC Deployment}
\label{subsec:eddcd}
...
\begin{figure*}
    \centering
    \includegraphics[width=1\columnwidth]{images/A_examlpe_of_DCC_placement1.png}  %ACM Journal
    \caption{Class 2-1: Examples of DCC insertion, one DCC on the common clock path (e.g., at buffer 1)}
    \label{fig:sub:dcci1}
\end{figure*}
\begin{figure*}
    \centering
    \includegraphics[width=1\columnwidth]{images/A_examlpe_of_DCC_placement2.png}  %ACM Journal
    \caption{Class 2-2: Examples of DCC insertion, one DCC on one of the divergent clock paths, or class 3: two DCCs, one on each of the divergent clock paths}
    \label{fig:sub:dcci2}
\end{figure*}
...
%(2) B
\section{Constraints on DCC Deployment}
\label{subsec:dccccc}
...
With DCC constraints and corresponding clauses, we can drastically reduce the possibilities of DCC insertions to be formulated. In the above example where the 48 clauses associated with DCC constraints are generated, the possibility count of DCC insertions drops from 16,384 to 103. In the next section, we describe what the 103 possibilities are and how they are translated into the final CNF representation.

%(3) C
\section{Timing Constraints}
\label{subsec:tccc}
Given a pair of flip-flops, if there exists one logic path between them, the timing (i.e. setup-time and hold-time) constraints must be met based on the inequalities~(\ref{eq:tsu}) and~(\ref{eq:th}). Different types of DCCs reveal different influence on clock latency. Consider the lifespan specification of 10 years in Figure~\ref{fig:sub:dcci2}: the delay of each clock buffer is changed after 10 years. The clock latency of $FF_i$ (i.e., $C_i$) is the sum of clock buffer delays from clock source to $FF_i$: 
\begin{gather*}
...
\textit{Class 2-2:} \mbox{ Inserting one DCC on one of the divergent clock paths}
...
\section{Additive Improvement in Aging Tolerance: High-$V_{th}$ Assignment for Clock Buffers}
\label{sec:VTA}
In this section, the approach of high-$V_{th}$ assignment for clock buffers is incorporated in the proposed framework to further improve the aging tolerability of designs. Because the fragmentary high-$V_{th}$ assignment for clock buffers may decrease the feasibility/manufacturability of the proposed framework, we make a rule/definition:
...
\section{Boolean Encoding for high-$V_{th}$ Buffer Leaders and DCCs}
...
\section{Constraints on High-$V_{th}$ Assignment for Clock Buffers}
\begin{comment}
\begin{figure}
	\centering
	\includegraphics[width=0.8\columnwidth]{All_types_of_DCCs_and_leaders.png} 
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=1\columnwidth]{images/2DCC_2Leader.png} 
